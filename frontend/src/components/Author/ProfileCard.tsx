...
export default function ProfileCard({ name, postCount, id }: ProfileCardProps) {
  const [currentTab, setCurrentTab] = useState("Home");
  // console.log("ProfileCard Props:", { name, postCount });

  const determineTabContent = () => {
    switch (currentTab) {
      case "Home":
        return postCount === 0 ? <NoBlogsMessage /> : <AuthorHome />;
      case "About":
        return <AuthorAbout />;
      default:
        return <></>;
  ...
  return (
    <div className="flex flex-row justify-center">
      ...
      <div className="border-1 border-slate-100 hidden md:block w-2/6 p-8 pr-36">
        ...
        <div className="text-sm mt-3 bg-red-500">{id}</div>
        <div className="text-sm mt-3 bg-red-500">{postCount}</div>
      </div>
    </div>
  );
}
